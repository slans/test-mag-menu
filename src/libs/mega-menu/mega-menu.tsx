...
export function MenuItem({ selected, label, mouseEntered, hasData, props, selectedProps }: menuItemProps) {
	return (
		<li {...props} {...(selected ? selectedProps : {})} onMouseEnter={mouseEntered}>
			{label}
		</li>
	);
}
...
export class ReactMegaMenu extends React.Component<Props, State> {
	...
	render() {
		...
		const { containerProps, menuProps, contentProps, menuItemSelectedProps, menuItemProps } = styleConfig;
		return (
			<div
				{...containerProps}
				className={`${(containerProps && containerProps.className) || ''} ${styles.flex} ${
			...
			>
				<ul
					{...menuProps}
					className={`${(menuProps && menuProps.className) || ''} ${styles.menuUl}`}
					ref={this.instance}
				...
				{activeRow > -1 && data[activeRow].items && (
					<div
						...
						className={`${(contentProps && contentProps.className) || ''} ${styles.contentSubMenu}`}
					>
